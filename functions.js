...
let classroom = {
  ...
  teacher: {
    name: "Mr J.", // is the only reason this teachers name is an object within an object to show that you can nest an object within an object or is there a reason why the only teacher for this class room needs an object of his own for a single name? :)
  },
  students: [
    {
      id: 11211,
...
function getStudentById(classroom, studentId) {
  let student = classroom.students.find(function (student) {
 ...
 */
function findAssignmentByAssignmentId(classroom, assignmentId) {
  let assignment = classroom.assignments.find(function (assignment) {
...
function calculateGradesForAssignmentId(classroom, assignmentId) {
  let assignment = findAssignmentByAssignmentId(classroom, assignmentId);
  let assignmentIndex = classroom.assignments.indexOf(assignment);
  classroom.students.forEach(function (student, index) {
    if (!classroom.students[index].grades[assignmentIndex]) {
      let grade = calculateAssignmentGradeForStudent(student, assignment);
      classroom.students[index].grades[assignmentIndex] = grade;
    }
  });
 ...
 * The above parameters in this example should be in the opposite order according to the function below.
...

{
...

  {
    ...
    calculateGradesForAssignmentId(
      testClassroom,
      testClassroom.assignments[0].id
    );
...

  {
    ...
    calculateGradesForAssignmentId(
      testClassroom,
      testClassroom.assignments[assignmentIndex].id
    );
...

  {
    ...
    console.log(
      testClassroom.assignments.includes(assignment) && everyStudentHasAGrade
    );
  }

  {
    ...
    teachAndGradeAssignment(
      testClassroom,
      testClassroom.assignments[assignmentIndex]
    );
    ...
    console.log(
      testClassroom.assignments.length === numAssignments &&
        everyStudentHasAGrade
    );
  }
...

  {
    ...
    let avgGrade = findAverageGradeForAssignmentId(
      testClassroom,
      assignment.id
    );
